...
<script>
import tabs from '../../../course/template/tabs'
import lists from '../../../course/template/lists'
...
export default {
  ...
  methods: {
    ...
    getCourseList () {
      let userId = 67;
      let liveId = 2;
      fly.get(`/api/live/v1/course/${userId}/${liveId}`).then((res)=>{
        let { code, data, message } = res.data;
        if (code === 200 && data) {
          console.log(data);
          this.checkList(data.liveCourseDetailVoList);
        }
      }).catch(err=>{
        console.error(err);
      })
