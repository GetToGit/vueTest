...
export default {
  ...
  methods: {
    ...
    getMistakeList () {
      ...
      let chapterId = 0;
      let curPage = 1;
      fly.get(`/api/mistake/v1/index/${userId}/${paperType}/${subjectId}`).then((res) => {
        let { code, message, data } = res.data;
        if (code === 200) {
          this.items = data.chapterVoList;
    ...
    getCollectList () {
      ...
      let chapterId = 0;
      let curPage = 1;
      fly.get(`/api/qa-favorite/v1/index/${userId}/${paperType}/${subjectId}`).then((res) => {
        let { code, message, data } = res.data;
        if (code === 200) {
          this.items = data.chapterVoList;
          this.total = data.totalCount;
    ...
    getWritingList () {
      ...
      let curPage = 1;
      let paperType = this.tabItems[this.tabIndex].id;
      fly.get(`/api/note/v1/index/${userId}/${paperType}/${subjectId}`).then((res) => {
        let { code, message, data } = res.data;
        if (code === 200) {
          this.items = data.chapterVoList;
          this.total = data.totalCount;
